...
Функция ЗагрузитьМарки()  Экспорт 
	...
	Для Каждого Элемент Из сткРезультат.масДанные Цикл
		...
		ШтрихКод = ПолучитьШтрихКод(ИДНоменклатуры, Марка);
...
Функция ПолучитьШтрихКод(ИДНоменклатуры,Марка)
	...
	если ИДНоменклатуры <> "" Тогда
		спр.Номенклатура = Справочники.Номенклатура.ПолучитьСсылку(Новый УникальныйИдентификатор(ИДНоменклатуры));
	КонецЕсли;
